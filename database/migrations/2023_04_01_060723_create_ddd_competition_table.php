...
return new class() extends Migration {
    ...
    {
        Schema::create('ddd_competition', static function (Blueprint $table): void {
            $table->id('incrementalId');
            $table->uuid('id')->nullable(false)->index()->unique();
            $table->string('name', 255)->nullable(false)->index();
            $table->string('description', 255)->nullable(false)->index();
            $table->dateTime('from')->nullable(false)->index();
            $table->dateTime('to')->nullable(false)->index();
    ...
    {
        Schema::dropIfExists('ddd_competition');
    }
};
